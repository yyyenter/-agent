...
class MemoryManager:  # 短期记忆与工具缓存
    """工业级 Session 记忆管理器 (CC架构)"""
    
    def __init__(self, session_id: str):
        self.session_id = session_id
        self.chat_key = f"session:{session_id}:chat"          # 桶3: 原始对话轮次
        self.summary_key = f"session:{session_id}:summary"    # 桶5: 短期精炼摘要
        self.cache_key = "cc:global:tool_cache"  # 桶4: 工具返回缓存
    ...
    def add_message(self, role: str, content: str, max_turns: int = 8):
        ...
        redis_client.rpush(self.chat_key, msg)
        # 只保留最近 max_turns * 2 条记录 (一问一答算1轮)
        redis_client.ltrim(self.chat_key, -(max_turns * 2), -1)
        redis_client.expire(self.chat_key, self.ttl)

    def save_chat(self, role: str, content: str):
        msg = json.dumps({"role": role, "content": content}, ensure_ascii=False)
        redis_client.rpush(self.chat_key, msg)
        redis_client.ltrim(self.chat_key, -10, -1) # 仅保留最近 5 轮

    @staticmethod
    def get_tool_result(tool_name: str, params: dict):
        param_hash = hashlib.md5(json.dumps(params, sort_keys=True).encode()).hexdigest()
        key = f"tool_cache:{tool_name}:{param_hash}"
        return redis_client.get(key)

    @staticmethod
    def set_tool_result(tool_name: str, params: dict, result: str, expire=3600):
        param_hash = hashlib.md5(json.dumps(params, sort_keys=True).encode()).hexdigest()
        key = f"tool_cache:{tool_name}:{param_hash}"
    ...
    def get_chat_history(self) -> list[dict]:
        """获取最近的原始对话"""
        raw = redis_client.lrange(self.chat_key, 0, -1)
        return [json.loads(m) for m in raw]

    # ==================== 桶5: Short-Term Memory (短期摘要) ====================
    def update_short_term_summary(self, new_constraints: dict):
        """
        更新短期摘要（如用户在第3轮突然说“我不想爬山”）
        new_constraints: {"preferences": "不爬山", "budget": "穷游"}
        """
        if new_constraints:
            redis_client.hset(self.summary_key, mapping=new_constraints)
            redis_client.expire(self.summary_key, self.ttl)

    def get_short_term_summary(self) -> dict:
        """获取当前会话的所有核心约束"""
        return redis_client.hgetall(self.summary_key)

    # ==================== 桶4: Tool Results (工具缓存) ====================
    def get_tool_cache(self, tool_name: str, query: str) -> str | None:
        """获取工具缓存，避免重复调用 API"""
        query_hash = hashlib.md5(query.encode()).hexdigest()
        cache_field = f"{tool_name}:{query_hash}"
        return redis_client.hget(self.tool_cache_key, cache_field)

    def set_tool_cache(self, tool_name: str, query: str, result: str, expire_seconds: int = 3600):
        """保存工具调用结果，默认缓存1小时"""
        query_hash = hashlib.md5(query.encode()).hexdigest()
        cache_field = f"{tool_name}:{query_hash}"
        redis_client.hset(self.tool_cache_key, cache_field, result)
        redis_client.expire(self.tool_cache_key, expire_seconds)

    # ==================== 组装全局 Context ====================
    def get_global_context_prompt(self, current_instruction: str) -> str:
        """
        组装发给大模型（用于重写或规划）的全局 Context 字符串
        """
        summary = self.get_short_term_summary()
        history = self.get_chat_history()
        
        # 将 Hash 转为易读文本
        summary_text = "、".join([f"{k}:{v}" for k, v in summary.items()]) if summary else "暂无"
        history_text = "\n".join([f"{msg['role']}: {msg['content']}" for msg in history])
        
        # 严格按照 CC 结构拼接
        context_str = f"""
            【当前行程核心约束 (Short-Term Memory)】
            {summary_text}
